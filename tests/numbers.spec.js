...
describe('2 - Implemente os casos de teste para a função `numbers`', () => {
  it('Verifica se a função `numbers` retorna `true` quando o array contém apenas números e falso caso contrário', () => {
    ...
    expect(numbers(arrNums)).toBe((true));
    // Escreva um teste em que a função recebe [1, 2, '3', 4, 5] e retorna false
    const numsAndString = [1, 2, '3', 4, 5];
    expect(numbers(numsAndString)).toBe((false));
    // Escreva um teste em que a função recebe [1, 'a', 3] e retorna false
    const numsAndStrings = [1, 'a', 3];
    expect(numbers(numsAndStrings)).toBe((false));
    // Escreva um teste em que a função recebe [' '] e retorna false
    const strings = [' '];
    expect(numbers(strings)).toBe((false))
  });
});
